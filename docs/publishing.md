...
1. Update `package.json` and `VERSION` together, add the `CHANGELOG.md` section headed `## <VERSION>` (optionally ` - YYYY-MM-DD`) with a summary paragraph followed by one bullet per change, and pass `bun run check`, independent review, and the protected pull request checks. Merge the task-owned change.
...
5. `publish` verifies the GitHub attestation signatures, hosted runner certificate, repository, tag source, workflow, exact run/attempt, and all four subjects, then renders the release page and creates the draft, uploads the five assets, checks provider digests, and publishes immutable Latest.
...
The release page follows `RELEASES.md` in hraness/.github. Its title is `Soulscrape v<VERSION>`. Its body is the summary and `## Changes` bullets copied from the version's section of `CHANGELOG.md` in the tagged commit, then `## Install` and `## Verify` generated from the release manifest, then the identity record as the final bytes: an HTML comment that opens with `<!-- soulscrape-release` and holds the `Source:`, `Workflow:`, and `Run:` lines. `verify` and `publish` fail before any release exists when that section is missing, empty, still says Unreleased, or lacks a summary paragraph and bullets. Every readback parses the identity from the last `<!-- soulscrape-release` marker, requires the body to end with `-->`, and requires the notes above it to match the rendered changelog section and generated sections byte for byte, so a hand edit to a published page fails the retry and mirror checks. GitHub's generated notes are never used.
