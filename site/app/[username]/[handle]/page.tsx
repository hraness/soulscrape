  ...
  return (
    <>
      ...
      <main className="person-main" id="main" tabIndex={-1}>
        <article className="person-body readme-prose">{renderMarkdown(packet.body)}</article>
      ...
      <footer className="site-footer person-footer">
        <p>
          ...
          <a href={`/${profile.username}`}>{`@${profile.username}`}</a> ·{" "}
          <a href={`${canonical}.md`}>Markdown</a> · <a href={siteReportUrl()}>Report an issue</a>
        </p>
      </footer>
    </>
  );
...
function siteReportUrl(): string {
  return "https://github.com/hraness/soulscrape/issues";
}
