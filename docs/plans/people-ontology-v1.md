...
- Setup: PeopleBlade's starting manifest omitted Convex while its lockfile retained it. With explicit user approval, `bun install` reconciled the lockfile; the diff removes the stale `convex` root entry and a dangling `prettier` record and re-resolves the esbuild platform packages to 0.28.2 — build tooling only, no runtime dependency changes. `bun install --frozen-lockfile` then passed in the isolated worktree.
...
- Validation evidence: Soulscrape root `bun run check` 355 pass; site `bun run check` green (95 tests incl. new graph-api, corpus-api, dossier-view, profile-links suites); PeopleBlade focused 121 pass plus full `bun run check` green; Sponge focused 54 pass incl. real two-CLI joins for Eugene Tssui (301 Oh records) and 37signals (485), both `authority: "unasserted"`; Sponge clean-clone `bun run check` pending at integration close.
