...
- Say what each published format carries. The web page at `/<username>/<handle>` shows the whole index, the JSON packet carries every claim and source, and the Markdown copy at `/<username>/<handle>.md` carries only the essay.
- Show commands that run as written. `publish-person.ts publish` and `validate-person-index.ts` accept only an absolute path, so write `"$PWD/examples/…"` or `/absolute/path/…`. A transcript shows the output the current CLI prints.
- The packet exporters are PeopleBlade and Message Like Me, the legacy message-history CLI kept in the Textbutler repository. Textbutler itself does not export packets.
- Decorative layers such as the hero field and its tape show real data from the example packets. Never show personality scores, trait charts, or invented claims about people; the skill forbids them.
- Example card notes and `subject.summary` lines claim only what the packet's own claims support, at the same strength and scope. Write “Spotify's most-streamed artist, 2020 to 2022”, not “the most-streamed artist alive”.
- Public dossier prose follows the writing rules in `skills/soulscrape/references/public-person-index.md`. Write each dossier from its sources; never copy wording, headings, or closing lines from another example's `generate.ts`.
