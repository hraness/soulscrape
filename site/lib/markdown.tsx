...
export function renderMarkdown(source: string, depth = 0): ReactNode {
  ...
  while (index < lines.length && blocks.length < MAX_BLOCKS) {
    ...
    if (heading !== null) {
      ...
      blocks.push(createElement(`h${level}`, { key: key++ }, ...inline(heading[2]!)));
      index += 1;
      continue;
    }
    ...
    if (list !== null) {
      ...
      blocks.push(<Fragment key={key++}>{renderListItems(items, () => key++)}</Fragment>);
      continue;
    }
    ...
    blocks.push(<p key={key++}>{...inline(collected.join(" ").trim())}</p>);
  }
  return <>{blocks}</>;
}
...
function renderListItems(items: readonly ListItem[], nextKey: () => number): ReactNode {
  ...
  if (nodes.length === 0) return renderListItems(orphans, nextKey);
  ...
  const list = createElement(
    ...
    ...nodes.map(node => (
      <li key={nextKey()}>
        {...inline(node.item.text)}
        {node.children.length > 0 && min < MAX_LIST_DEPTH * 4
          ? renderListItems(node.children, nextKey)
          : node.children.flatMap(child => inline(child.text))}
      </li>
    )),
  );
  return orphans.length === 0
    ? list
    : <>{renderListItems(orphans, nextKey)}{list}</>;
}
