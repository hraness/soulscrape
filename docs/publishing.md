...
3. `verify` checks the committed support helper against its pinned build inputs, runs the tagged product gates, packs once, and tests the exact archive through the npm/Bun installed-payload smoke. It imports hash-verified package and release helpers from current `main`; the tagged release workflow must equal current-main authority.
