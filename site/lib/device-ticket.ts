...
export function deviceCodeDigest(code: string): string {
  return sha256Hex(`soulscrape.device-code.v1\n${code}`);
}

export function deviceSecretDigest(secret: string): string {
  return sha256Hex(`soulscrape.device-secret.v1\n${secret}`);
}

export function publishTokenDigest(token: string): string {
  return sha256Hex(`soulscrape.publish-token.v1\n${token}`);
}
...
export function mintDeviceAuthorizeTicket(input: {
  ...
  const payload = `${input.code}.${input.accountId}.${input.username}.${expiresAtMs}`;
