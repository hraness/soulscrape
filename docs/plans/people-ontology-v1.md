...
- Follow-up hardening: Soulscrape keeps the frozen v1 parser byte-compatible but compares offsets and arbitrary fractional precision chronologically before producing any new research exchange (PR #64, `59f8457`). PeopleBlade local schema v21 materializes the reviewed identity-component projection, updates ordinary accepts from indexed component state, recomputes only affected sides after separation, and leaves list/stat reads non-recursive (PR #165, `c6bab65`). Sponge measures the actual deduplicated Oh packet against the 1,024-record ceiling and represents non-NFC claims through an explicit NFC text projection with the original UTF-8 bytes and digest retained in context (PR #290, `6402240`); its preview-theme browser gate now waits on concrete product readiness instead of `networkidle`.
- Follow-up validation: Soulscrape `bun run check` passed with 357 tests and unchanged 29-file package inventory. PeopleBlade focused identity/research/migration tests, the 50-surface cost registry, typecheck, complete local `bun run check`, packed install, and integrated-head release gates passed. Sponge focused exchange/adapter/conformance/CLI/docs tests and complete local `bun run check` passed; integrated-head CI is the delivery gate of record. Frozen historical exchanges remain readable, no existing person-index digest changed, and none of these follow-ups grants factual, review, rights, identity, or publication authority.
- Remaining risks: `soulscrape.person-index.v1` itself retains its historical lexical timestamp rule for compatibility; new research exports cannot carry a chronological inversion. PeopleBlade pays target-scoped recomputation on rare separation decisions and a one-time full projection during schema-v21 migration instead of repeated whole-graph work on reads. Sponge may still reject a valid exchange when its actual deduplicated Oh packet exceeds 1,024 records or an explicit lossless text projection exceeds an Oh field bound; it never truncates either case.
